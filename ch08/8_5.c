...
int fseek_ex(register FILE *fp, register long offset, int origin);
...
int fseek_ex(register FILE *fp, register long offset, int origin)
{
  ...
  if (fp->_flag & _READ) {
    if (origin == 1) 
      offset -= fp->_cnt;
    rc = lseek(fp->_fd, offset, origin);
    fp->_cnt = 0;
  } else if (fp->_flag & _WRITE) {
    if ((nc = fp->_ptr - fp->_base) > 0)
      if (write(fp->_fd, fp->_base, nc) != nc)
        rc = -1;
    if (rc != -1)
      rc = lseek(fp->_fd, offset, origin);    
  }
